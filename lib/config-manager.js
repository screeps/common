...
exports.load = function() {
    if(!process.env.MOD_DIR) {
        throw new Error('MOD_DIR environment variable is not set!');
    }
    var modsJsonFilename = path.resolve(process.env.MOD_DIR, 'mods.json');
    try {
    ...
    try {
        ...
        modsJson.mods.forEach(file => {
            file = path.resolve(process.env.MOD_DIR, file);
            try {
